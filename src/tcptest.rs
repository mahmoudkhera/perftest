...
impl StreamTester {
 ...
 pub fn set_mss(stream: tokio::net::TcpStream, mss: u32) -> tokio::net::TcpStream {
    const IPPROTO_TCP: i32 = 6; // always 6 for TCP
    ...
    #[cfg(target_os = "linux")]
    {
        ...
        let ret = unsafe {
            libc::setsockopt(
                ...
                IPPROTO_TCP,                                   //protocol leve
                TCP_MAXSEG,                                    //option name
                &mss as *const _ as *const libc::c_void,       //pointer to option name
                std::mem::size_of::<i32>() as libc::socklen_t, //size of option value
            )
        };
        if ret == -1 {
    ...
    #[cfg(target_os = "windows")]
    {
        ...
        let socket = std_stream.as_raw_socket() as usize;
        let ret = unsafe {
            libc::setsockopt(
                socket,                            //socket file descriptor
                IPPROTO_TCP,                       //protocol leve
                TCP_MAXSEG,                        //option name
                &mss as *const _ as *const i8,     //pointer to option name
                std::mem::size_of::<i32>() as i32, //size of option value
            )
        };

        // Verify MSS with getsockopt

        unsafe {
            let mut actual_mss: i32 = 0;
            let mut len = std::mem::size_of::<i32>() as i32;
            libc::getsockopt(
                socket,
                IPPROTO_TCP,
                TCP_MAXSEG,
                &mut actual_mss as *mut _ as *mut _,
                &mut len,
            );

            println!("MSS successfully set. Current MSS = {}", actual_mss);
        }
        if ret == -1 {
            println!("setsockopt failed ");
        }
